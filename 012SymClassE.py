...
def probe_function(x):
    return x[0]*x[1]/ ( (x[2]-x[3])**2  +      (x[4]-x[5])**2                  )
...
def explicit_gradient(x):
    return [probe_function(x)/x[0] , probe_function(x)/x[1]  , - 2*(x[2]-x[3])* probe_function(x)**2/(x[0]*x[1])  ,  - 2*(x[3]-x[2])* probe_function(x)**2/(x[0]*x[1]) ,- 2*(x[4]-x[5])* probe_function(x)**2/(x[0]*x[1]),- 2*(x[5]-x[4])* probe_function(x)**2/(x[0]*x[1]) ]

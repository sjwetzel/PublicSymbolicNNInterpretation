...
def explicit_gradient(x):
    return [3*np.cos(3*(x[0]+x[1])),3*np.cos(3*(x[0]+x[1])),0,0]

def explicit_gradient2(x):
    return [0,0,1 + 4*x[2]+x[3],1 + 4*x[2]+x[3]]
...
plt.scatter(predictions_latent,x[:,0]*x[:,1]+x[:,2]*x[:,3],label="latent model")
...
ax1.scatter(predictions_latent[:data_fraction], y_probe[:data_fraction], label='$x_1*x_2-1$', alpha=0.4, s=6, c='blue')
ax1.scatter(predictions_latent[:data_fraction], y_probe2[:data_fraction], label='$x_3*x_4-1$', alpha=0.4, s=6, c='green')
...
ax2 = ax1.twinx()
ax2.scatter(predictions_latent[:data_fraction], x[:data_fraction,0]*x[:data_fraction,1]+x[:data_fraction,2]*x[:data_fraction,3], label='$x_1*x_2+x_3*x_4$', alpha=0.4, s=6, c='red')
